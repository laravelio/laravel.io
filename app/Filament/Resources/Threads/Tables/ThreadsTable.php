    ...
    public static function configure(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('solutionReplyRelation.body')
                    ->label('Solution')
                    ->limit(300)
                    ->wrap()
                    ->searchable(),
                ...
                ImageColumn::make('resolvedByRelation.github_id')
                    ->label('Resolved by')
                    ->circular()
                    ->defaultImageUrl(fn(?string $state): string => $state ? sprintf('https://avatars.githubusercontent.com/u/%s', $state) : asset('images/laravelio-icon-gray.svg')),

                TextColumn::make('resolvedByRelation.name')
                    ->label('')
                    ->sortable()
                    ->searchable(),


                ImageColumn::make('updatedByRelation.github_id')
                    ->label('Updated by')
                    ->circular()
                    ->defaultImageUrl(fn(?string $state): string => $state ? sprintf('https://avatars.githubusercontent.com/u/%s', $state) : asset('images/laravelio-icon-gray.svg')),

                TextColumn::make('updatedByRelation.name')
                    ->label('')
                    ->sortable()
                    ->searchable(),

                ImageColumn::make('lockedByRelation.github_id')
                    ->label('Locked by')
                    ->circular()
                    ->defaultImageUrl(fn(?string $state): string => $state ? sprintf('https://avatars.githubusercontent.com/u/%s', $state) : asset('images/laravelio-icon-gray.svg')),

                TextColumn::make('lockedByRelation.name')
                    ->label('')
                    ->sortable()
                    ->searchable(),
