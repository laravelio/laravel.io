    ...
    <div class="content">
        ...
        <ul class="meta">
            ...
            @if(Auth::check() && $comment->author_id == Auth::user()->id)
                <li><i class="icon-link"></i>&nbsp;<a href="{{ action('ArticlesController@getEditComment', [$comment->id]) }}">Edit</a></li>
                {{-- <li><i class="icon-link"></i>&nbsp;<a href="{{ action('ArticlesController@getDelete', [$comment->id]) }}">Delete</a></li> --}}
            @endif
        </ul>
    </div>
</div>