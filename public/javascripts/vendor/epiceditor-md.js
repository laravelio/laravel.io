...
//
  ...
  text = text.replace(/https?\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!]/g, function(wholeMatch,matchIndex){
    ...
    return "<a target='blank' href='" + wholeMatch + "'>" + wholeMatch + "</a>";
  });
  ...
  text = text.replace(/[a-f0-9]{40}/ig, function(wholeMatch,matchIndex){
    ...
    return "<a target='blank' href='http://github.com/" + GitHub.nameWithOwner + "/commit/" + wholeMatch + "'>" + wholeMatch.substring(0,7) + "</a>";
  });
  ...
  text = text.replace(/([a-z0-9_\-+=.]+)@([a-f0-9]{40})/ig, function(wholeMatch,username,sha,matchIndex){
    ...
    var left = text.slice(0, matchIndex), right = text.slice(matchIndex)
    if (left.match(/\/$/) || (left.match(/<[^>]+$/) && right.match(/^[^>]*>/))) {return wholeMatch;}
    return "<a target='blank' href='http://github.com/" + username + "/" + GitHub.repoName + "/commit/" + sha + "'>" + username + "@" + sha.substring(0,7) + "</a>";
  });

  // ** GFM ** Auto-link user/repo@sha1
  text = text.replace(/([a-z0-9_\-+=.]+\/[a-z0-9_\-+=.]+)@([a-f0-9]{40})/ig, function(wholeMatch,repo,sha){
    return "<a target='blank' href='http://github.com/" + repo + "/commit/" + sha + "'>" + repo + "@" + sha.substring(0,7) + "</a>";
  });
  ...
  text = text.replace(/#([0-9]+)/ig, function(wholeMatch,issue,matchIndex){
    ...
    return "<a target='blank' href='http://github.com/" + GitHub.nameWithOwner + "/issues/#issue/" + issue + "'>" + wholeMatch + "</a>";
  });
  ...
  text = text.replace(/([a-z0-9_\-+=.]+)#([0-9]+)/ig, function(wholeMatch,username,issue,matchIndex){
    ...
    var left = text.slice(0, matchIndex), right = text.slice(matchIndex)
    if (left.match(/\/$/) || (left.match(/<[^>]+$/) && right.match(/^[^>]*>/))) {return wholeMatch;}
    return "<a target='blank' href='http://github.com/" + username + "/" + GitHub.repoName + "/issues/#issue/" + issue + "'>" + wholeMatch + "</a>";
  });

  // ** GFM ** Auto-link user/repo#issue
  text = text.replace(/([a-z0-9_\-+=.]+\/[a-z0-9_\-+=.]+)#([0-9]+)/ig, function(wholeMatch,repo,issue){
    return "<a target='blank' href='http://github.com/" + repo + "/issues/#issue/" + issue + "'>" + wholeMatch + "</a>";
  });
...
var writeAnchorTag = function(wholeMatch,m1,m2,m3,m4,m5,m6,m7) {
  ...
  var result = "<a target='blank' href=\"" + url + "\"";
...
var _DoAutoLinks = function(text) {
  ...
  text = text.replace(/<((https?|ftp|dict):[^'">\s]+)>/gi,"<a target='blank' href=\"$1\">$1</a>");
...
//
  ...
  addr = "<a target='blank' href=\"" + addr + "\">" + addr + "</a>";
