            ...
            @if ($thread->isLocked())
                <x-info-panel class="flex justify-between gap-x-16">
                    <p class="text-sm">
                        ...
                        <a class="text-lio-500 hover:text-lio-600"
                            href="{{ route('profile', $lockedBy->username()) }}">{{ $lockedBy->name() }}</a>.
                    </p>
                </x-info-panel>
            @endif
            ...
            @can(App\Policies\ReplyPolicy::CREATE, App\Models\Reply::class)
                @if ($thread->isUnlocked() || Auth::user()->isModerator() || Auth::user()->isAdmin())
                    @if ($thread->isConversationOld())
                        <x-info-panel class="flex justify-between gap-x-16">
                            <p>The last reply to this thread was more than six months ago. Please consider opening a new thread
                                if you have a similar question.</p>
                    ...
                    @else
                        <div class="my-8">
                            <form action="{{ route('replies.store') }}" method="POST"
                                ...
                                <livewire:editor hasMentions hasButton buttonLabel="Reply" buttonIcon="send"
                                    label="Write a reply" :participants="$thread->participants()" />
                                    ...
                                    <div class="mt-4 flex items-start justify-between gap-x-8 lg:items-center">
                                        <p>
                                            Please make sure you've read our <a href="{{ route('rules') }}"
                                                class="border-b-2 border-lio-100 pb-0.5 text-lio-500 hover:text-lio-600">rules</a>
                ...
                @else
                    @guest
                        <p class="py-8 text-center">
                            <a href="{{ route('login') }}" class="border-b-2 border-lio-100 pb-0.5 text-lio-500 hover:text-lio-600">Sign
                                in</a> to participate in this thread!
                        </p>
                    @else
                        <x-info-panel>
