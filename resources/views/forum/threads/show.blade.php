...
@section('content')
    <section class="container mx-auto flex flex-col gap-x-12 px-4 pb-10 pt-5 lg:flex-row lg:pb-0 lg:pt-10">
        <div class="w-full lg:w-3/4">
            ...
            @if ($thread->isLocked())
                <x-info-panel class="flex justify-between gap-x-16">
                    <p class="text-sm">
                        ...
                        <a class="text-lio-500 hover:text-lio-600" href="{{ route('profile', $lockedBy->username()) }}">{{ $lockedBy->name() }}</a
                        >.
                    </p>
                </x-info-panel>
            @endif
            ...
            @else
                @guest
                    <p class="py-8 text-center"><a href="{{ route('login') }}" class="border-b-2 border-lio-100 pb-0.5 text-lio-500 hover:text-lio-600">Sign in</a> to participate in this thread!</p>
                @else
                    <x-info-panel>
