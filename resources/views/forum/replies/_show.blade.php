<div class="comment {{ $thread->isReplyTheSolution($reply) ? 'solution-border' : '' }} _post" id="reply-{{ $reply->id }}" data-author-name='{{ json_encode($reply->author->name, JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS) }}' data-quote-body='{{ json_encode($reply->getWrappedObject()->body, JSON_HEX_QUOT|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS) }}'>
        ...
        <div class="admin-bar">
            <ul>
                @if ($reply->isManageableBy(Auth::user()))
                    <li><a href="{{ action('Forum\ForumRepliesController@getEditReply', $reply->id) }}">Edit</a></li>
                    <li><a href="{{ action('Forum\ForumRepliesController@getDelete', $reply->id) }}">Delete</a></li>
                @endif
