                                    ...
                                    <tr>
                                        ...
                                        <x-tables.table-data class="w-10 text-center">
                                            ...
                                            @can(App\Policies\UserPolicy::DELETE, $user)
                                                <button @click="activeModal = 'deleteUser{{ $user->getKey() }}'"
                                                    class="text-red-600 hover:text-red-800">
                                                    <x-heroicon-o-trash class="inline h-5 w-5" />
                                                </button>

                                                <x-modal identifier="deleteUser{{ $user->getKey() }}" :action="route('admin.users.delete', $user->username())"
                                                    title="Delete {{ $user->username() }}">
                                                    <p>Deleting this user will remove their account and any related content like
                                                        threads & replies. This cannot be undone.</p>
                                                </x-modal>
                                            @endcan
                                        </x-tables.table-data>
                                    </tr>
