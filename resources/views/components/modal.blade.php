            ...
            <x-buk-form :action="$action" :method="$method()">
                <div class="flex h-full flex-col justify-between">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="mr-4 text-gray-600" @click.prevent="activeModal = false">Cancel</button>

                        <button type="submit" class="button {{ $type === 'delete' ? 'button-danger' : 'button-primary' }}">
                            {{ $submit ?? $title }}
                        </button>
                    </div>
                </div>
