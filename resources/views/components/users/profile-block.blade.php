...
<div class="mt-6 flex flex-col rounded-md shadow">
    <div
        class="h-28 rounded-t-md bg-gray-800"
        style="
    ...
    <div
        class="flex flex-col items-center justify-center rounded-b-md bg-white pb-8"
    >
        <x-avatar :user="$user" class="-mt-16 mb-6 h-32 w-32 rounded-full" />

        <a
            href="{{ route('profile', $user->username()) }}"
            class="mb-4 flex flex-col text-center text-xl font-medium hover:underline"
        >
            {{ $user->name() }}
