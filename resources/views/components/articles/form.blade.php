...
<x-buk-form action="{{ route(...$route) }}" :method="$method">
    <div class="space-y-6 bg-gray-100 px-4 py-6 sm:p-6">
        <div>
            ...
            <x-info class="mt-4">
                Submit your article to the Laravel.io portal. We're looking for high quality articles revolving around
                Laravel, PHP, JavaScript, CSS, and related topics. Articles can't be promotional in nature and should be
                educational and informative. We reserve the right to decline articles that don't meet our quality
                standards.
            </x-info>

            <x-info class="mt-4">
                Every article that gets approved will be shared with our 50.000 users and wil be tweeted out on our <a
                    href="https://twitter.com/laravelio" class="text-lio-700 underline">Twitter account</a> which has over
                50,000 followers. Feel free to submit as many articles as you like. You can even cross-reference an
                article on your blog with the original url.
            </x-info>

            <x-info class="mt-4">
                After submission for approval, articles are reviewed before being published. No notification of declined
                articles will be provided. Instead, we encourage to also cross-post articles on your own channel as
                well. <strong>After being published, you cannot edit your article anymore so please review it thoroughly
                    before submitting for approval.</strong> Submitting the same article twice or posting spam will
                result in the banning of your account.
            </x-info>
                ...
                <div class="grow space-y-6">
                    ...
                    <div class="grow space-y-6">
                        <div class="space-y-1">
                            ...
                            <select name="tags[]" id="create-article" multiple x-data="{}"
                                ...
                                @foreach ($tags as $tag)
                                    <option value="{{ $tag->id }}"
                                        {{ in_array($tag->id, $selectedTags) ? 'selected' : '' }}>{{ $tag->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="md:flex md:items-center md:justify-between">
                        ...
                        <div class="flex items-center justify-end">
                            <a href="{{ isset($article) ? route('articles.show', $article->slug()) : route('user.articles') }}"
                            ...
                            @else
                                <span class="relative z-0 inline-flex shadow-sm" x-data="{ showDropdown: false }"
                                    ...
                                    <span class="relative -ml-px block">
                                        ...
                                        <div class="absolute right-0 mt-2 w-60 origin-top-right rounded-md shadow-lg"
