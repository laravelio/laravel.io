...
<div class="h-full rounded-lg bg-white shadow-lg lg:p-5">
    <div class="flex flex-col gap-x-8 lg:flex-row">
        ...
        <div class="flex w-full flex-col gap-y-3 p-4 lg:gap-y-3.5 lg:p-0">
            <div>
                <div
                    class="flex flex-col justify-between gap-y-2 lg:flex-row lg:items-center"
                >
                ...
                @if (isset($mode) && $mode == 'edit')
                    <div class="flex text-sm leading-5 text-gray-500">
                        @if ($article->isPublished())
                            <time
                                datetime="{{ $article->submittedAt()->format('Y-m-d') }}"
                            >
                                Published
                        ...
                        @else
                            ...
                            @else
                                <time
                                    datetime="{{ $article->updatedAt()->format('Y-m-d') }}"
                                >
                                    Drafted
                                    {{ $article->updatedAt()->format('j M, Y') }}
                                </time>
                            @endif
                        @endif
                    </div>
            ...
            <div
                class="flex flex-col gap-y-3 lg:flex-row-reverse lg:items-center lg:justify-between"
            >
                <div>
