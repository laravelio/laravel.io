    ...
    <div class="relative -mr-3" x-data="{ open: false }" @click.outside="open = false">
        ...
        >                
            ...
            @can(App\Policies\ArticlePolicy::UPDATE, $article)
                <a class="flex gap-x-2 p-3 rounded-sm hover:bg-gray-100" href="{{ route('articles.edit', $article->slug()) }}">
                    <x-heroicon-o-pencil class="w-6 h-6"/>
                    Edit
                </a>
            @endcan

            <a class="flex gap-x-2 p-3 rounded-sm hover:bg-gray-100" href="{{ route('articles.show', $article->slug()) }}">
