...
@if ($reply->trashed())
    <div class="bg-gray-50">
        <div>
        @else
            <div class="@if (!$reply->trashed() && $thread->isSolutionReply($reply)) border-2 border-lio-400 @endif rounded bg-white shadow"
                id="{{ $reply->id() }}" x-data="{ edit: false }">
                <div class="border-b">
@endif
<div class="flex flex-row items-center justify-between px-6 py-2">
    <div class="flex flex-wrap items-center space-x-1 text-sm">
        @if ($reply->trashed())
            <div>
                <div
                    class="-ml-2 mr-2 flex h-8 w-8 items-center justify-center rounded-full border border-gray-200 bg-white lg:ml-6">
        ...
        <div class="inline-block">
            <div class="flex items-center">
                <x-avatar :user="$reply->trashed() ? $reply->remover() : $reply->author()" class="mr-2 h-6 w-6 rounded-full" />

                <a href="{{ route('profile', ($reply->trashed() ? $reply->remover() : $reply->author())->username()) }}"
                    class="hover:underline">
                    <span
                        class="font-semibold text-gray-900">{{ ($reply->trashed() ? $reply->remover() : $reply->author())->username() }}</span>
                </a>
            </div>
        </div>
        ...
        <a href="#{{ $reply->id() }}" class="pl-8 text-gray-700 hover:text-lio-500 hover:underline sm:pl-0">
    ...
    <div class="flex justify-between" x-show="!edit">
        ...
        @if ($thread->isSolutionReply($reply) && ($resolvedBy = $thread->resolvedBy()))
            <div class="px-6 pb-6 text-lio-500">
                ...
                <a href="{{ route('profile', $resolvedBy->username()) }}" class="font-bold text-lio-600 hover:text-lio-800">
                    {{ '@' . $resolvedBy->username() }}
                </a>
            </div>
        @endif
