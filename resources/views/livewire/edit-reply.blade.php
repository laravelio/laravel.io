<div>
    ...
    $wire.on('replyEdited', function() { show = true;
        edit = false; });" class="prose-lio prose max-w-none break-words p-6">
        {!! replace_links(md_to_html($reply->body())) !!}
    </div>

    @can(App\Policies\ReplyPolicy::UPDATE, $reply)
        <div x-show="edit" @submitted.stop="$wire.updateReply($event.detail.body)">
            <livewire:editor x-cloak :hasShadow="false" :body="$reply->body()" hasMentions hasButton buttonLabel="Update reply"
                buttonIcon="heroicon-o-arrow-right" cancelAction='@click="edit = false; open = false;"' />
