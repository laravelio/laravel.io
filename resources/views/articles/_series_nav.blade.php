@if ($article->series && $article->isPublished())
    <div class="flex items-center mt-12">
        @if ($previous = $article->previousInSeries())
            <a href="{{ route('articles.show', $previous->slug()) }}" class="flex justify-start w-full items-center text-green-600">
        ...
        @if ($next = $article->nextInSeries())
            <a href="{{ route('articles.show', $next->slug()) }}" class="flex justify-end items-center w-full text-green-600">
                {{ $next->title() }}
