...
@section('content')
    <article class="bg-white">
        @auth
            ...
            @elseif ($article->isPublished() && $article->isAuthoredBy(Auth::user()))
                <x-info-banner>
                    Your article is now published and cannot be edited anymore. If you want to perform any changes to the
                    article, please email <a href="mailto:hello@laravel.io">hello@laravel.io</a>
                </x-info-banner>
            @endif
        @endauth

        <div class="{{ $article->hasHeroImage() ? 'bg-cover' : '' }} w-full bg-gray-800 bg-center"
        ...
        <div class="container mx-auto">
            <div class="flex px-4 lg:mx-48 lg:px-0">
                ...
                <div class="w-full pt-4 lg:w-4/5 lg:pt-10">
                    ...
                    <div x-data="{}" x-init="$nextTick(function() { highlightCode($el); })" class="prose-lio prose prose-lg text-gray-800">
                    ...
                    <div class="border-t-2 border-gray-200 py-8 lg:pb-16 lg:pt-14">
                        <div
                            class="flex flex-col items-center justify-center gap-x-6 gap-y-4 lg:flex-row lg:justify-between">
    ...
    @can(App\Policies\ArticlePolicy::APPROVE, $article)
        @if ($article->isAwaitingApproval())
            <x-modal identifier="approveArticle" :action="route('admin.articles.approve', $article->slug())" title="Approve article" type="update">
    ...
    @can(App\Policies\ArticlePolicy::DISAPPROVE, $article)
        @if ($article->isPublished())
            <x-modal identifier="unpublishArticle" :action="route('admin.articles.disapprove', $article->slug())" title="Unpublish article" type="update">
    ...
    @can(App\Policies\ArticlePolicy::DECLINE, $article)
        @if ($article->isNotDeclined())
            <x-modal identifier="declineArticle" :action="route('admin.articles.decline', $article->slug())" title="Decline article" type="update">
    ...
    @can(App\Policies\ArticlePolicy::DELETE, $article)
        <x-modal identifier="deleteArticle" :action="route('articles.delete', $article->slug())" title="Delete article">
    ...
    @can(App\Policies\ArticlePolicy::PINNED, $article)
        <x-modal identifier="togglePinnedStatus" :action="route('admin.articles.pinned', $article->slug())" :title="$article->isPinned() ? 'Unpin article' : 'Pin article'" type="update">
            @if ($article->isPinned())
