...
@section('content')
    <section class="bg-white">
        <div class="h-60 w-full bg-gray-800 bg-center"
            style="background-image: url('{{ asset('images/default-background.svg') }}')"></div>

        <div class="container mx-auto">
            <div class="flex justify-center lg:justify-start">
                <x-avatar :user="$user" class="-mt-24 h-48 w-48 rounded-full border-8 border-white" unlinked />
            </div>

            <div class="mt-5 flex flex-col p-4 lg:flex-row lg:gap-x-12">
                <div class="mb-10 w-full lg:mb-0 lg:w-1/3">
                    <div>
                        <div class="flex items-center gap-x-4">
                            <h1 class="text-4xl font-bold">{{ $user->name() }}</h1>

                            @if ($user->isAdmin() || $user->isModerator())
                                <span class="rounded border border-lio-500 px-3 py-1 text-lio-500">
                    ...
                    <div class="mb-6 mt-4 flex items-center gap-x-3">
                    ...
                    <div class="flex flex-col gap-y-4">
                        ...
                        @if ($user->bannedReason())
                            @can(App\Policies\UserPolicy::BAN, $user)
                                <div class="mt-2 rounded border border-red-200 bg-red-100 px-4 py-3 text-sm shadow">
                ...
                <div class="w-full lg:w-2/3">
                    ...
                    <div class="mt-4 grid grid-cols-1 lg:grid-cols-2">
                        <div class="flex w-full justify-between bg-gray-100 px-5 py-2.5">
                            <span>Threads</span>
                        ...
                        <div class="flex w-full justify-between bg-white px-5 py-2.5 lg:bg-gray-100">
                            <span>Replies</span>
                            <span class="text-lio-500">
                        ...
                        <div class="flex w-full justify-between bg-gray-100 px-5 py-2.5 lg:bg-white">
                            <span>Solutions</span>
                            <span class="text-lio-500">
                        ...
                        <div class="flex w-full justify-between px-5 py-2.5">
                            <span>Articles</span>
                            <span class="text-lio-500">
            ...
            @if ($articles->count() > 0)
                <div class="mt-10 px-4 lg:mt-28">
                    ...
                    <div class="mt-8 flex flex-col gap-y-8 lg:mb-16 lg:flex-row lg:gap-x-8">
        ...
        <div class="mt-16 lg:mt-32" x-data="{ tab: 'threads' }">
            <div class="container mx-auto">
                <nav class="flex items-center justify-between lg:justify-start">
                    <button @click="tab = 'threads'"
                        ...
                        class="whitespace-nowrap px-4 py-5 text-lg font-medium text-gray-900 hover:border-lio-500 hover:text-lio-500 focus:border-lio-500 focus:text-lio-500 focus:outline-none lg:w-1/3">
                        Threads posted
                    </button>
            ...
            <div class="bg-gray-100 px-4 py-14">
                <div class="container mx-auto">
    ...
    @can(App\Policies\UserPolicy::BLOCK, $user)
        @if (Auth::user()->hasBlocked($user))
            <x-modal identifier="unblockUser" :action="route('users.unblock', $user->username())" title="Unblock {{ $user->username() }}" type="update">
                <p>Unblocking this user will allow them to mention you again in threads and replies.</p>
            </x-modal>
        @else
            <x-modal identifier="blockUser" :action="route('users.block', $user->username())" title="Block {{ $user->username() }}" type="update">
                <p>Blocking this user will prevent them from mentioning you in threads and replies. The user will not be
                    notified that you blocked them.</p>
            </x-modal>
        @endif
    @endcan
