...
@section('content')
    <section class="bg-white">
        <div class="h-60 w-full bg-gray-800 bg-center" style="background-image: url('{{ asset('images/default-background.svg') }}')"></div>
            ...
            <div class="mt-5 flex flex-col p-4 lg:flex-row lg:gap-x-12">
                <div class="mb-10 w-full lg:mb-0 lg:w-1/3">
                    <div>
                        ...
                        <span class="text-gray-600">Joined {{ $user->createdAt()->format('j M Y') }}</span>
                    </div>
    ...
    @can(App\Policies\UserPolicy::BLOCK, $user)
        ...
        @else
            <x-modal identifier="blockUser" :action="route('users.block', $user->username())" title="Block {{ $user->username() }}" type="update">
                <p>
                    Blocking this user will prevent them from mentioning you in threads and replies. The user will not be notified that you blocked
                    them.
                </p>
            </x-modal>
        @endif
